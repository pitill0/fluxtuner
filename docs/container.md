...
The web interface plays streams in the browser, so the container does not need
...
podman run --rm --name fluxtuner-web -p 8080:8080 -e FLUXTUNER_DATA_DIR=/data -v fluxtuner-data:/data fluxtuner-web:dev
```
...
docker run --rm --name fluxtuner-web -p 8080:8080 -e FLUXTUNER_DATA_DIR=/data -v fluxtuner-data:/data fluxtuner-web:dev
```
...
See `docs/web.md` for the general web interface documentation.
