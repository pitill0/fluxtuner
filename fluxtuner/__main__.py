...
def set_web_user_password(username: str) -> None:
...
def deactivate_web_user(username: str) -> None:
...
def handle_web_user_command(command: list[str]) -> bool:
    ...
    if len(command) == 4 and command[:3] == ["web", "users", "set-password"]:
        set_web_user_password(command[3])
        return True

    if len(command) == 4 and command[:3] == ["web", "users", "deactivate"]:
        deactivate_web_user(command[3])
        return True
